...
def crop(x, out_shape):
    ...
    out_shape = cp.array([s if s > 0 else in_shape[i] for i, s in enumerate(out_shape)])
...
def downsample(stack, n, mask=None, stack_in_fourier=False):
    ...
    size_in = cp.square(stack.shape[1])
    size_out = cp.square(n)
    mask = 1 if mask is None else mask
    num_images = stack.shape[0]
    output = cp.zeros((num_images, n, n), dtype='float64')
    images_batches = cp.array_split(cp.arange(num_images), 500)
    for batch in images_batches:
        if batch.size:
            curr_batch = cp.array(stack[batch])
            curr_batch = curr_batch if stack_in_fourier else fft2(curr_batch)
            fx = crop(cp.fft.fftshift(curr_batch, axes=(-2, -1)), (-1, n, n)) * mask
            output[batch] = ifft2(cp.fft.ifftshift(fx, axes=(-2, -1))) * (size_out / size_in)
    return output
...
def icfft(x, axis=0):
    return cp.fft.fftshift(cp.fft.ifft(cp.fft.ifftshift(x, axis), axis=axis), axis)


def fast_cfft2(x, axes=(-1, -2)):
    if len(x.shape) == 2:
        return cp.fft.fftshift(cp.transpose(fft2(cp.transpose(cp.fft.ifftshift(x)))))
    elif len(x.shape) == 3:
        y = cp.fft.ifftshift(x, axes=axes)
        y = fft2(y, axes=axes)
        y = cp.fft.fftshift(y, axes=axes)
...
def fast_icfft2(x, axes=(-1, -2)):
    if len(x.shape) == 2:
        return cp.fft.fftshift(cp.transpose(ifft2(cp.transpose(cp.fft.ifftshift(x)))))
...
def cryo_epsdr(vol, samples_idx, max_d):
    ...
    tmp[:p, :p] = mask
    ftmp = fft2(tmp)
    c = ifft2(ftmp * cp.conj(ftmp))
    c = c[:max_d + 1, :max_d + 1]
    c = cp.round(c.real).astype('int')
...
def cryo_prewhiten(proj, noise_response, rel_threshold=None):
    ...
    filter_var = cp.sqrt(noise_response)
    filter_var /= cp.linalg.norm(filter_var)

    filter_var = (filter_var + cp.flipud(filter_var)) / 2
    filter_var = (filter_var + cp.fliplr(filter_var)) / 2

    if rel_threshold is None:
        nzidx = cp.where(filter_var > 100 * delta)
    else:
    ...
    one_over_fnz_as_mat = cp.zeros((noise_response.shape[0], noise_response.shape[1]))
    one_over_fnz_as_mat[nzidx] += one_over_fnz
    pp = cp.zeros((noise_response.shape[0], noise_response.shape[1]))
    p2 = cp.zeros((num_images, L1, L2), dtype='complex128')
    ...
    for i in range(num_images):
        ...
        pp2 = fast_icfft2(fp)
        p2[i] = cp.real(pp2[row_start_idx:row_end_idx, col_start_idx:col_end_idx])
...
def als_find_min(sreal, eps, max_iter):
    ...
    sz = sreal.shape
    patch_num = sz[1]
    One = cp.ones(patch_num)
    s_norm_inf = cp.apply_along_axis(lambda x: max(cp.abs(x)), 0, sreal)
    max_col = cp.argmax(s_norm_inf)
    min_col = cp.argmin(s_norm_inf)
    clean_sig_tmp = cp.abs(sreal[:, max_col] - sreal[:, min_col])
    s_norm_1 = cp.apply_along_axis(lambda x: sum(cp.abs(x)), 0, sreal)
    min_col = cp.argmin(s_norm_1)
    noise_sig_tmp = cp.abs(sreal[:, min_col])
    s = sreal - cp.outer(noise_sig_tmp, One)
    alpha_tmp = (cp.dot(clean_sig_tmp, s)) / cp.sum(clean_sig_tmp ** 2)
    alpha_tmp = alpha_tmp.clip(min=0, max=1)
    stop_par = 0
    cnt = 1
    while stop_par == 0:
        if not cp.linalg.norm(alpha_tmp, 1):
            alpha_tmp = cp.random.random(alpha_tmp.size)
        approx_clean_psd = cp.dot(s, alpha_tmp) / sum(alpha_tmp ** 2)
        approx_clean_psd = approx_clean_psd.clip(min=0, max=None)
        s = sreal - cp.outer(approx_clean_psd, alpha_tmp)
        approx_noise_psd = cp.dot(s, cp.ones(patch_num)) / patch_num
        approx_noise_psd = approx_noise_psd.clip(min=0, max=None)
        s = sreal - cp.outer(approx_noise_psd, One)
        if not cp.linalg.norm(approx_clean_psd, 1):
            approx_clean_psd = cp.random.random(approx_clean_psd.size)
        alpha_approx = cp.dot(approx_clean_psd, s) / sum(approx_clean_psd ** 2)
        alpha_approx = alpha_approx.clip(min=0, max=1)
        if cp.linalg.norm(noise_sig_tmp - approx_noise_psd) / cp.linalg.norm(approx_noise_psd) < eps:
            if cp.linalg.norm(clean_sig_tmp - approx_clean_psd) / cp.linalg.norm(approx_clean_psd) < eps:
                if cp.linalg.norm(alpha_approx - alpha_tmp) / cp.linalg.norm(alpha_approx) < eps:
                    break
...
def picking_from_scoring_mat(log_test_n, mrc_name, kltpicker, mg_big_size):
    idx_row = cp.arange(log_test_n.shape[0])
    idx_col = cp.arange(log_test_n.shape[1])
    [col_idx, row_idx] = cp.meshgrid(idx_col, idx_row)
    r_del = cp.floor(kltpicker.patch_size_pick_box)
    shape = log_test_n.shape
    scoring_mat = log_test_n
    if kltpicker.num_of_particles == -1:
        num_picked_particles = write_output_files(scoring_mat, shape, r_del, cp.iinfo(cp.int32(10)).max, op.gt,
                                                  kltpicker.threshold + 1, kltpicker.threshold,
                                                  kltpicker.patch_size_func,
                                                  row_idx, col_idx, kltpicker.output_particles, mrc_name,
                                                  kltpicker.mgscale, mg_big_size, -cp.inf,
    ...
    if kltpicker.num_of_noise_images != 0:
        num_picked_noise = write_output_files(scoring_mat, shape, r_del, kltpicker.num_of_noise_images, op.lt,
                                              ...
                                              kltpicker.mgscale, mg_big_size, cp.inf, kltpicker.patch_size_pick_box)
    else:
        num_picked_noise = 0
...
def write_output_files(scoring_mat, shape, r_del, max_iter, oper, oper_param, threshold, patch_size_func, row_idx,
    ...
    while iter_pick <= max_iter and oper(oper_param, threshold):
        ...
        if not oper(oper_param, threshold):
            ...
            box_file.write(
                '%i\t%i\t%i\t%i\n' % ((1 / mgscale) * (ind_col_patch + 1 - cp.floor(patch_size_pick_box / 2)),
                                      (mg_big_size[0] + 1) - (1 / mgscale) * (
                                                  ind_row_patch + 1 + cp.floor(patch_size_pick_box / 2)),
